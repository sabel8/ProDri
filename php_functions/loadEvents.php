...
for ($i=0; $i < $days; $i++) {			
	...
	\"{$begin->format('d')}\" as day,time(events.startTime),addTime(time(events.startTime),duration),events.nodeID as nodeID
		...
		WHERE ((year(startTime)=".intval($begin->format('Y'))." AND month(startTime)=".intval($begin->format('n'))." AND day(startTime)=".
		intval($begin->format('d')).")
...
WHERE personID=$userID AND UNIX_TIMESTAMP(startTime)<=".strtotime($begin->format('Y-m-d')));
